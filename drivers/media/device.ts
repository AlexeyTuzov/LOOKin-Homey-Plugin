  ...
  async onInit() {
    ...
    this.registerCapabilityListener('button.mode', async () => {
      if (!(this.getStoreValue('functions').find((item: Functions) => item.Name === 'mode'))) {
        this.homey.app.error('No mode change command found! Please, create it in LOOKin APP first!');
        throw new Error('No mode change command found! Please, create it in LOOKin APP first!');
      }
      let changeMode = await httpRequest(IP, `${path}04FF`);
      if (JSON.parse(changeMode).success === 'false') {
        this.homey.app.error('Failed to change the mode! No connection to remote');
        throw new Error('Failed to change the mode! No connection to remote');
      }
    });
